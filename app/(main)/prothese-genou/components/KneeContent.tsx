...
interface KneeSection {
  title: string;
  content?: ContentItem[];
  items?: string[];
}

interface KneeContentProps {
  kneeData: Record<string, KneeSection>;
  sectionRefs: Record<string, RefObject<HTMLElement | null>>;
}

const KneeContent = ({ kneeData, sectionRefs }: KneeContentProps) => {
  return (
    <article className="col-span-12 lg:col-span-8 pb-24">
      {Object.entries(kneeData).map(([key, section]) => (
        <motion.section
          ref={sectionRefs[key]}
          key={key}
          data-section={key}
