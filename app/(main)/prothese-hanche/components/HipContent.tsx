...
interface HipSection {
  title: string;
  content?: ContentItem[];
  items?: string[];
}

interface HipContentProps {
  hipData: Record<string, HipSection>;
  sectionRefs: Record<string, RefObject<HTMLElement | null>>;
}

const HipContent = ({ hipData, sectionRefs }: HipContentProps) => {
  return (
    <article className="col-span-12 lg:col-span-8 pb-24">
      {Object.entries(hipData).map(([key, section]) => (
        <motion.section
          ref={sectionRefs[key]}
          key={key}
          data-section={key}
          ...
          {"content" in section && section.content ? (
                ...
                >
                  ...
                  <div className="absolute inset-0">
                    <TexturedBackground
                      ...
                      baseFrom="from-purple-100/60"
                      baseVia="via-brand-bay-of-many-100/30"
                      baseTo="to-violet-100/20"
                      spotOneColor="bg-violet-100/50"
                      spotTwoColor="bg-purple-100/20"
                      noiseOpacity={70}
                      noiseContrast={110}
            ...
            <div className="relative rounded-lg overflow-hidden p-6 shadow-sm border border-brand-bay-of-many-100/40">
              ...
              <div className="absolute inset-0">
                <TexturedBackground
                  className="absolute inset-0"
                  baseFrom="from-violet-100/60"
                  baseVia="via-brand-bay-of-many-100/30"
                  baseTo="to-purple-100/20"
                  spotOneColor="bg-violet-100/30"
                  spotTwoColor="bg-purple-100/20"
                  noiseOpacity={30}
                  noiseContrast={120}
                />
