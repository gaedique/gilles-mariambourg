...
// import { ComponentWithDisplayName, CtaButtonProps, IconProps } from "./types";
...
// Instead of passing component references, pass icon names as strings
const CtaButton = ({
  href,
  iconName, // Change from icon: Icon to iconName
  children,
  variant = "dark",
  fullWidth = false,
  external = false,
  ...
  disabled = false,
  onClick,
}: Omit<CtaButtonProps, "icon"> & { iconName?: string }) => {
  // State to track the ringing animation
  const [isRinging, setIsRinging] = useState(false);
  const [IconComponent, setIconComponent] = useState<ElementType | null>(null);

  // Dynamically load the icon based on name
  useEffect(() => {
    const loadIcon = async () => {
      if (!iconName) return;

      try {
        // Use a safer approach with specific imports
        const icons = await import("lucide-react");

        // Use type assertion and indexing with keyof and typeof
        type IconKey = keyof typeof icons;
        // Check if the iconName exists as a key in icons
        if (iconName in icons) {
          const Icon = icons[iconName as IconKey];
          setIconComponent(() => Icon as ElementType);
        } else {
  ...
  // Check if the icon is a Phone icon by its name
  const isPhoneIcon = iconName === "Phone";
  ...
  const renderContent = () => {
    ...
    const iconElement = IconComponent && (
      <div
        className={`
        ${isPhoneIcon && isRinging ? "animate-phone-ringing" : ""}
        transition-all duration-300
        ${isPhoneIcon ? "relative" : ""}
      `}
      >
        {/* Optional background circle for phone icon */}
        {isPhoneIcon && (
    ...
    return (
      <div className="flex items-center gap-2">
        {IconComponent && iconPosition === "left" && iconElement}
        <span
          className={`
            ${variant === "light" ? "text-brand-bay-of-many-800" : ""}
            font-medium
        ...
        {IconComponent && iconPosition === "right" && iconElement}
      </div>
    );
  };
